...
namespace Netron.GraphLib.UI
{
	...
	{
      public  ContextMenuStrip ThisMenuStrip;
		#region Constants
		...
		{		
            ...
            ThisMenuStrip = new ContextMenuStrip();
            AutoScroll = true;
            RestrictToCanvas = false;
            this.ShowGrid = false;

            ThisMenuStrip = new ContextMenuStrip();
            ThisMenuStrip.Items.Add("É¾³ý").Click += (s, e) =>
            {
                OnDelete(this, EventArgs.Empty); 

            };
           
            ContextMenuStrip = ThisMenuStrip;
		...
		protected override void OnMouseDown(MouseEventArgs e)
		{
				...
				if ((e.Button == MouseButtons.Left) && (e.Clicks == 1))
				{
					...
					if (Hover != null)//the click resulted in an object; shape or connection
					{
						if (typeof(Connector).IsInstanceOfType(Hover))  
						{
							...
							if(!(Hover as Connector).AllowNewConnectionsFrom) return;
			...
			else //allow the adornments only
			{
				if ((e.Button == MouseButtons.Left) && (e.Clicks == 1))
				{
					...
					if (Hover != null)//the click resulted in an object; shape or connection
					{
						if (typeof(Entity).IsInstanceOfType(Hover))
						{
							(Hover as Entity).RaiseMouseDown(e);
						}
