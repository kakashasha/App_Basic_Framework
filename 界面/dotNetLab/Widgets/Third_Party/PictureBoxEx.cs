...
namespace dotNetLab.Widgets.Third_Party
{
    ...
    public  class PictureBoxEx : UserControl
    {
        ...
        private void PictureBoxEx_MenuStripItem_Click(object sender, EventArgs e)
        {
            ToolStripItem item = sender as ToolStripItem;
            switch (item.Text)
            {
                case "缩放":AllawDraw = false;  break;
                case "绘制矩形": AllawDraw = true; break;
                case "设置矩形颜色":
                    ColorDialog clrDlg = new ColorDialog();
                    if(clrDlg.ShowDialog()== DialogResult.OK)
                    {
                        this.RectColor = clrDlg.Color;
                    }
                    clrDlg.Dispose();
                    break;
            }
        ...
        public void SetImage(Image bitmap, bool isFirst, bool isDeleteRect = false, float zoom = 1, PointF centerpoint = default(PointF))
        {
            ...
            _image = (System.Drawing.Image)bitmap.Clone();
           // _image = bitmap;
            if (isFirst)
            {
                if (IsFirstZoom)
